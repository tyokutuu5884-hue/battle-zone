...
function btn(variant = "primary", sm = false) {
  ...
  const variants = {
    ...
    red:     { background: "#3a0a0a", color: C.red,  border: "1.5px solid #f8717144" },
...
function Toast({ toast }) {
  ...
  return (
    <div style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", background: toast.type === "error" ? "#3a0a0a" : "#0a2a1a", color: toast.type === "error" ? "#dc2626" : "#16a34a", border: `1px solid ${toast.type === "error" ? "#f87171" : "#34d399"}`, borderRadius: 10, padding: "10px 24px", fontSize: 13, zIndex: 9999, boxShadow: "0 8px 24px #0000001a", maxWidth: 380, textAlign: "center", pointerEvents: "none", fontWeight: 600 }}>
...
function ConfirmDialog({ dialog, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0006", zIndex: 8000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#2a1010", borderRadius: 16, padding: "28px 24px", maxWidth: 360, width: "100%", boxShadow: "0 20px 60px #0003" }}>
        ...
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[[dialog.p1name, dialog.r1after, dialog.diff1], [dialog.p2name, dialog.r2after, dialog.diff2]].map(([name, after, diff], i) => (
              <div key={i} style={{ flex: 1, textAlign: "center", background: "#1a0808", borderRadius: 8, padding: "10px 6px", border: `1px solid ${C.border}` }}>
...
function MatchTimer({ matchId, timerStartedAt, onStart, canControl }) {
  ...
  const bgColor = over ? "#2a0000" : warn ? "#2a1800" : "#002a10";
  const borderColor = over ? "#e74c3c" : warn ? "#e67e22" : "#2ecc71";
...
function AuthScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "'Hiragino Sans','Yu Gothic UI',sans-serif", background: "linear-gradient(135deg,#1a0505 0%,#2d0a0a 50%,#1a0808 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ width: 80, height: 80, margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center" }}><img src="/logo.png" alt="POWERS" style={{ width: 80, height: 80, objectFit: "contain", borderRadius: 12 }} /></div>
        ...
        <div style={{ background: "#2a1010", borderRadius: 16, padding: "28px 24px", boxShadow: "0 4px 24px #0009", border: `1px solid ${C.border}` }}>
          <div style={{ display: "flex", background: C.bg, borderRadius: 10, padding: 4, marginBottom: 22 }}>
            {[["login","ログイン"],["register","新規登録"]].map(([m,l]) => (
              <button key={m} onClick={() => { setMode(m); setError(""); }} style={{ flex: 1, padding: "8px", borderRadius: 7, border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700, background: mode === m ? "#5a1e1e" : "transparent", color: mode === m ? C.accent : C.muted, boxShadow: mode === m ? "0 1px 4px #0000001a" : "none" }}>{l}</button>
            ))}
          </div>
          ...
          {error && <div style={{ background: "#3a0a0a", border: "1px solid #f8717155", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: C.red, marginBottom: 14, fontWeight: 600 }}>⚠️ {error}</div>}
...
function TournamentLobby({ account, onEnter, onLogout }) {
  ...
  async function createTournament() {
    ...
    try {
      ...
      await api.addParticipant({ tournament_id: tour.id, account_id: account.id, rating_at_join: 1500, current_rating: 1500, wins: 0, losses: 0, draws: 0 });
      onEnter(tour);
    ...
    <div style={{ minHeight: "100vh", fontFamily: "'Hiragino Sans','Yu Gothic UI',sans-serif", background: "linear-gradient(135deg,#1a0505 0%,#2d0a0a 50%,#1a0808 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <div style={{ width: 64, height: 64, margin: "0 auto 10px", display: "flex", alignItems: "center", justifyContent: "center" }}><img src="/logo.png" alt="POWERS" style={{ width: 64, height: 64, objectFit: "contain", borderRadius: 10 }} /></div>
          <div style={{ fontSize: 20, fontWeight: 900, color: C.text, letterSpacing: 1 }}>BATTLE ZONE</div>
        </div>

        {/* Account chip */}
        <div style={{ background: "#2a1010", borderRadius: 12, padding: "12px 16px", marginBottom: 16, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 4px #0000000a" }}>
          <img src="/logo.png" alt="POWERS" style={{ width: 40, height: 40, objectFit: "contain", borderRadius: 8, flexShrink: 0 }} />
          <div style={{ flex: 1 }}>
        ...
        {mode === "top" && (
          <div style={{ background: "#2a1010", borderRadius: 16, padding: "24px", boxShadow: "0 4px 24px #0009", border: `1px solid ${C.border}` }}>
...
function TournamentApp({ account, setAccount, tournament, onLeave }) {
  ...
  const played = new Set(recentMatches.map(m => pairKey(m.p1_id, m.p2_id)));
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Hiragino Sans','Yu Gothic UI',sans-serif", color: C.text }}>
      ...
      <div style={{ background: "#200a0a", borderBottom: `1px solid ${C.border}`, padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 1px 4px #0000000a", flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <img src="/logo.png" alt="POWERS" style={{ width: 30, height: 30, objectFit: "contain", borderRadius: 6 }} />
          <div>
        ...
        <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
          {Array.from({ length: tableCount }, (_, i) => i + 1).map(t => (
            <div key={t} style={{ width: 26, height: 26, borderRadius: 6, fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", background: usedTables.has(t) ? "#fde8e8" : "#f0fdf4", color: usedTables.has(t) ? C.accent : C.green, border: `1.5px solid ${usedTables.has(t) ? "#f87171" : "#86efac"}` }}>{t}</div>
          ))}
        </div>
      ...
      {tab === "queue" && (
        <div style={S.body}>
          {!myPart
            ? <div style={{ ...S.card, borderColor: "#7f1d1d", background: "#3a0a0a", marginBottom: 12 }}><div style={{ fontSize: 13, color: C.red, fontWeight: 700 }}>⚠️ あなたはこの大会に参加していません</div></div>
          ...
          <div style={{ ...S.card, borderColor: "#6b2a2a", background: "#2a1010", marginBottom: 14 }}>
          ...
          {[...participants].sort((a, b) => {
            ...
            return (
              <div key={p.id} style={{ ...S.card, borderColor: isMe ? "#f87171" : st === "playing" ? "#86efac" : st === "waiting" ? "#fde047" : C.border, borderWidth: isMe ? 2 : 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      <span style={{ fontWeight: 700, fontSize: 14 }}>{p.account.name}</span>
                      {isMe && <span style={{ fontSize: 10, fontWeight: 800, color: "#f87171", background: "#3a1010", border: "1px solid #7a3030", borderRadius: 4, padding: "1px 6px" }}>YOU</span>}
      ...
      {tab === "tables" && (
        <div style={S.body}>
          ...
          {[...pendingMatches].sort((a, b) => {
            ...
            return (
              {(() => { const isMyMatch = m.p1_id === account.id || m.p2_id === account.id; return (
              <div key={m.id} style={{ background: isMyMatch ? "#3a0808" : "#2a1212", border: `2px solid ${isMyMatch ? C.accent : C.border2}`, borderRadius: 14, padding: "18px 16px", marginBottom: 12, boxShadow: isMyMatch ? `0 0 16px ${C.accent}44` : "0 2px 8px #0000000a", position: "relative" }}>
      ...
      {tab === "ranking" && (
        <div style={S.body}>
          ...
          {ranked.filter(p => p.account.name.toLowerCase().includes(search.toLowerCase())).map((p, i) => {
            ...
            const isMe = p.account_id === account.id;
            return (
              <div key={p.id} style={{ ...S.card, borderColor: isMe ? "#f87171" : i === 0 ? t.border : C.border, borderWidth: isMe ? 2 : 1 }}>
            ...
            <div key={m.id} style={S.card}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#f87171", background: "#3a1010", border: "1px solid #7a3030", borderRadius: 12, padding: "2px 9px" }}>台{m.table_num}</span>
          ...
          <div style={S.body}>
            <div style={{ ...S.card, background: `linear-gradient(135deg,${t.bg},#fff)`, borderColor: t.border, marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <img src="/logo.png" alt="POWERS" style={{ width: 52, height: 52, objectFit: "contain", borderRadius: 10, flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
