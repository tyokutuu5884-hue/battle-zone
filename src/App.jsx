...
function TournamentApp({ account, setAccount, tournament, onLeave }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      const playingNow = new Set((pend || []).flatMap(m => [m.p1_id, m.p2_id]));
  ...
  function reportResult(matchId, result) {
    ...
    const side = isOperator && !isP1 && !isP2 ? "operator" : isP1 ? "p1side" : "p2side";
    const prev = matchReports[matchId] || {};
    const updated = { ...prev, [side]: result };
    setMatchReports(r => ({ ...r, [matchId]: updated }));
    ...
    // 両者報告が揃ったか確認
    const p1report = side === "p1side" ? result : prev.p1side;
    const p2report = side === "p2side" ? result : prev.p2side;

    if (p1report && p2report) {
      if (p1report === p2report) {
        // 一致 → 確定
        const scoreA = p1report === "p1" ? 1 : p1report === "draw" ? 0.5 : 0;
        const [nr1, nr2] = calcNewRatings(m.p1_rating_before, m.p2_rating_before, scoreA);
        const label = p1report === "p1" ? `${m.p1_name} 勝利` : p1report === "p2" ? `${m.p2_name} 勝利` : "対戦中止";
        setConfirm({ matchId, result: p1report, label, p1name: m.p1_name, p2name: m.p2_name, r1after: nr1, r2after: nr2, diff1: nr1 - m.p1_rating_before, diff2: nr2 - m.p2_rating_before, table: m.table_num });
      } else {
        // 不一致 → 運営に通知
        showToast(`台${m.table_num}: 報告が一致しません。運営に確認を依頼してください。`, "error");
      }
    } else {
      showToast(`報告を受け付けました。相手の報告を待っています...`);
    }
  }
            ...
            return (
              <div key={m.id} style={{ background: isMyMatch ? "#161a28" : "#12151f", border: `2px solid ${isMyMatch ? C.accent : C.border2}`, borderRadius: 14, padding: "18px 16px", marginBottom: 12, boxShadow: isMyMatch ? `0 0 16px ${C.accent}44` : "0 2px 8px #0000000a", position: "relative" }}>
                ...
                {(isMyMatch || isOperator) ? (
                  <div>
                    {(() => {
                      const reports = matchReports[m.id] || {};
                      const isP1 = m.p1_id === account.id;
                      const isP2 = m.p2_id === account.id;
                      const mySide = isP1 ? "p1side" : isP2 ? "p2side" : null;
                      const myReport = mySide ? reports[mySide] : null;
                      const otherReport = isP1 ? reports.p2side : isP2 ? reports.p1side : null;
                        ...
                        <div>
                          ...
                          {!isOperator && (
                            <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                              {[["p1side", m.p1_name], ["p2side", m.p2_name]].map(([side, name]) => (
                                <div key={side} style={{ flex: 1, textAlign: "center", fontSize: 11, padding: "5px 8px", borderRadius: 6, background: reports[side] ? "#001a10" : C.dim, border: `1px solid ${reports[side] ? C.green : C.border}`, color: reports[side] ? C.green : C.muted }}>
                                  {name}: {reports[side] ? resultLabel(reports[side]) : "未報告"}
                                </div>
                              ))}
                            </div>
                          )}
                          {/* 不一致警告 */}
                          {reports.p1side && reports.p2side && reports.p1side !== reports.p2side && (
