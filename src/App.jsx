                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      ...
                      {st === "waiting" && <span style={{ fontSize: 11, fontWeight: 700, color: C.orange, background: "#fffbeb", border: "1px solid #fde047", borderRadius: 12, padding: "2px 8px" }}>⏳ 待機 #{inQueue + 1}</span>}
                    </div>
                    <div style={{ fontSize: 12, color: C.muted }}>{p.current_rating} pt ／ {p.wins}勝 {p.losses}敗</div>
                  </div>
                  {st === "free"    && <button style={btn("green", true)} onClick={() => joinQueue(p.account_id)}>⚡ 対戦申請</button>}
                  {st === "waiting" && <button style={btn("red", true)} onClick={() => leaveQueue(p.account_id)}>キャンセル</button>}
...
export default function App() {
  ...
  return (
    <>
      ...
      {!account && <AuthScreen onLogin={setAccount} />}
      {account && !tournament && <TournamentLobby account={account} onEnter={setTournament} onLogout={() => setAccount(null)} />}
      {account && tournament && <TournamentApp account={account} setAccount={setAccount} tournament={tournament} onLeave={() => setTournament(null)} />}
    </>
  );
}
