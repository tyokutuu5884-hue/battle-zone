...
const S = {
  card: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 8, boxShadow: "0 1px 6px #00000010" },
...
function btn(variant = "primary", sm = false) {
  ...
  const variants = {
    ...
    red:     { background: "#fff0ee", color: C.red,  border: `1.5px solid ${C.red}44` },
...
function Toast({ toast }) {
  ...
  return (
    <div style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", background: toast.type === "error" ? "#fff5f5" : "#f0fdf8", color: toast.type === "error" ? "#dc2626" : "#16a34a", border: `1px solid ${toast.type === "error" ? "#f87171" : "#34d399"}`, borderRadius: 10, padding: "10px 24px", fontSize: 13, zIndex: 9999, boxShadow: "0 8px 24px #0000001a", maxWidth: 380, textAlign: "center", pointerEvents: "none", fontWeight: 600 }}>
...
function ConfirmDialog({ dialog, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0006", zIndex: 8000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#ffffff", borderRadius: 16, padding: "28px 24px", maxWidth: 360, width: "100%", boxShadow: "0 20px 60px #0003" }}>
        ...
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[[dialog.p1name, dialog.r1after, dialog.diff1], [dialog.p2name, dialog.r2after, dialog.diff2]].map(([name, after, diff], i) => (
              <div key={i} style={{ flex: 1, textAlign: "center", background: "#f4f5f7", borderRadius: 8, padding: "10px 6px", border: `1px solid ${C.border}` }}>
...
function MatchTimer({ matchId, timerStartedAt, onStart, canControl }) {
  ...
  const bgColor = over ? "#f5f0ff" : warn ? "#fffbf0" : "#f0f8ff";
  ...
  useEffect(() => {
    ...
    el.textContent = `
      ...
      @keyframes bz-glow-pulse {
        0%,100% { box-shadow: 0 0 10px #e8321a33, inset 0 0 0px #e8321a00; }
        50%      { box-shadow: 0 0 28px #e8321a66, 0 0 48px #ff440022, inset 0 0 12px #ff440011; }
      }
      ...
      @keyframes bz-vs-beat {
        0%,100% { transform: scale(1);   text-shadow: 0 0 8px #e8321aaa; }
        50%      { transform: scale(1.3); text-shadow: 0 0 22px #e8321aff, 0 0 40px #ff220088; }
      }
      ...
      @keyframes bz-winner-glow {
        0%,100% { box-shadow: 0 0 16px #e8321a55, 0 0 40px #ff220022; border-color: #e8321a; }
        50%      { box-shadow: 0 0 36px #e8321acc, 0 0 80px #ff440055; border-color: #ff8844; }
      }
      .bz-match-card-my {
...
function RatingAnimation({ data, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: "rgba(0,0,0,0.88)", backdropFilter: "blur(4px)" }}
      ...
      {/* 閃光フラッシュ */}
      {flashVisible && (
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle, #e8321a88 0%, transparent 70%)", animation: "bz-flash 0.4s ease-out forwards", pointerEvents: "none" }} />
      )}

      {/* スキャンライン演出 */}
      <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
        <div style={{ position: "absolute", left: 0, right: 0, height: 2, background: "linear-gradient(90deg,transparent,#e8321a44,transparent)", animation: "bz-scanline 2s linear infinite" }} />
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 380, textAlign: "center", position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 28 }}>
          {[
            ...
            <div key={i} style={{
              flex: 1, background: "#fff5f5", borderRadius: 16, padding: "20px 10px",
...
function AuthScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "'Hiragino Sans','Yu Gothic UI',sans-serif", background: "linear-gradient(135deg,#f0f2f6 0%,#e8eaf0 50%,#f0f2f6 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        <div style={{ background: "#ffffff", borderRadius: 16, padding: "28px 24px", boxShadow: "0 4px 16px #00000012", border: `1px solid ${C.border}` }}>
          ...
          {error && <div style={{ background: "#fff5f5", border: `1px solid ${C.red}55`, borderRadius: 8, padding: "8px 12px", fontSize: 12, color: C.red, marginBottom: 14, fontWeight: 600 }}>{error}</div>}
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "'Hiragino Sans','Yu Gothic UI',sans-serif", background: "linear-gradient(135deg,#f0f2f6 0%,#e8eaf0 50%,#f0f2f6 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        {/* Account chip */}
        <div style={{ background: "#ffffff", borderRadius: 12, padding: "12px 16px", marginBottom: 16, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 1px 4px #0000000a" }}>
        ...
        {mode === "top" && (
          <div>
            ...
            {myTournaments.length > 0 && (
              <div style={{ background: "#ffffff", borderRadius: 16, padding: "20px", marginBottom: 12, border: `1px solid ${C.border}`, boxShadow: "0 4px 16px #00000012" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.accent, letterSpacing: 1.2, marginBottom: 12 }}>参加中の大会</div>
                {myTournaments.map(tour => (
                  <button key={tour.id} onClick={() => onEnter(tour)} style={{ width: "100%", background: C.dim, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "12px 14px", marginBottom: 8, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ flex: 1 }}>
            ...
            {/* 新規作成・参加 */}
            <div style={{ background: "#ffffff", borderRadius: 16, padding: "24px", boxShadow: "0 4px 16px #00000012", border: `1px solid ${C.border}` }}>
...
function TournamentApp({ account, setAccount, tournament, onLeave }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Hiragino Sans','Yu Gothic UI',sans-serif", color: C.text }}>
      ...
      {/* Header */}
      <div style={{ background: "#1a1e2e", borderBottom: `1px solid #2a2e40`, padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 2px 8px #00000018", flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <img src="/logo.jpg" alt="POWERS" style={{ width: 30, height: 30, objectFit: "contain", borderRadius: 6 }} />
          <div>
            <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: 0.5, color: "#ffffff" }}>BATTLE ZONE</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.75)" }}>{tournament.name}</div>
          </div>
        </div>

        {/* Tournament ID - tap to copy */}
        {isOperator && <div onClick={copyId} style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.12)", borderRadius: 20, padding: "4px 12px", cursor: "pointer", userSelect: "none" }}>
          <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 2, color: "#ffffff" }}>ID: {tournament.id}</span>
          <span style={{ fontSize: 11, color: copied ? "#b8ffee" : "rgba(255,255,255,0.6)" }}>{copied ? "コピー済" : "ID"}</span>
        </div>}

        {/* Table indicators */}

        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 20, padding: "4px 12px" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#ffffff" }}>{account.name}</span>
          </div>
          <button style={{ ...btn("ghost", true), background: "rgba(255,255,255,0.15)", color: "#ffffff", border: "1px solid rgba(255,255,255,0.3)" }} onClick={onLeave}>退出</button>
        </div>
      </div>
      ...
      {tab === "queue" && (
        <div style={S.body}>
          {/* 自分のカード（一番上） */}
          {myPart ? (() => {
            const st = playerStatus(account.id);
            return (
              <div style={{ ...S.card, border: `2px solid ${C.accent}`, marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span style={{ fontWeight: 800, fontSize: 16, color: C.text }}>{account.name}</span>
                      <span style={{ fontSize: 10, fontWeight: 800, color: "#fff", background: C.accent, borderRadius: 4, padding: "1px 7px" }}>YOU</span>
                      {st === "playing" && <span style={{ fontSize: 11, fontWeight: 700, color: C.green, background: "#e8fff8", border: `1px solid ${C.green}`, borderRadius: 12, padding: "2px 8px" }}>対戦中</span>}
                      {st === "waiting" && <span style={{ fontSize: 11, fontWeight: 700, color: C.orange, background: "#fff8e8", border: `1px solid ${C.orange}`, borderRadius: 12, padding: "2px 8px" }}>待機中</span>}
                    </div>
                    <div style={{ fontSize: 13, color: C.sub, fontWeight: 600 }}>{myPart.current_rating} pt ／ {myPart.wins}勝 {myPart.losses}敗</div>
                  </div>
                  {st === "free"    && <button style={btn("primary")} onClick={() => joinQueue(account.id)}>対戦申請</button>}
                  {st === "waiting" && <button style={btn("red")} onClick={() => leaveQueue(account.id)}>キャンセル</button>}
                  {st === "playing" && <span style={{ fontSize: 13, fontWeight: 800, color: C.accent }}>台{pendingMatches.find(m => m.p1_id === account.id || m.p2_id === account.id)?.table_num}</span>}
                </div>
                {/* 連戦拒否設定 */}
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, marginBottom: 8, letterSpacing: 0.8 }}>連戦拒否設定</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {[0, 1, 2, 3, 5, 10].map(n => (
                      <button key={n} style={{ ...btn(myPart.rematch_limit === n || (n === 5 && myPart.rematch_limit == null) ? "primary" : "ghost", true) }} onClick={() => updateRematchLimit(n)}>
                        {n === 0 ? "なし" : `直近${n}戦`}
                      </button>
                    ))}
                  </div>
            ...
            <div style={{ ...S.card, borderColor: C.red, marginBottom: 12 }}>
              <div style={{ fontSize: 13, color: C.red, fontWeight: 700 }}>あなたはこの大会に参加していません</div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
            {Array.from({ length: tableCount }, (_, i) => i + 1).map(t => (
              <div key={t} style={{ padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 700, background: usedTables.has(t) ? "#fff0ee" : "#e8fff8", color: usedTables.has(t) ? C.orange : C.green, border: `1.5px solid ${usedTables.has(t) ? C.orange : C.green}` }}>
          ...
          {[...participants].sort((a, b) => b.current_rating - a.current_rating)
            .filter(p => p.account_id !== account.id)
            .filter(p => isOperator || playerStatus(p.account_id) !== "waiting")
            .map(p => {
              const st = playerStatus(p.account_id);
              return (
                <div key={p.id} style={{ ...S.card, borderColor: st === "playing" ? C.green : st === "waiting" ? C.orange : C.border }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: C.text }}>{p.account.name}</span>
                        {st === "playing" && <span style={{ fontSize: 11, fontWeight: 700, color: C.green, background: "#e8fff8", border: `1px solid ${C.green}`, borderRadius: 12, padding: "2px 8px" }}>対戦中</span>}
                        {st === "waiting" && isOperator && <span style={{ fontSize: 11, fontWeight: 700, color: C.orange, background: "#fff8e8", border: `1px solid ${C.orange}`, borderRadius: 12, padding: "2px 8px" }}>待機中</span>}
                      </div>
                      <div style={{ fontSize: 12, color: C.muted }}>{p.current_rating} pt ／ {p.wins}勝 {p.losses}敗</div>
                    </div>
                    {isOperator && st === "free"    && <button style={btn("green", true)} onClick={() => joinQueue(p.account_id)}>対戦申請</button>}
                    {isOperator && st === "waiting" && <button style={btn("red", true)} onClick={() => leaveQueue(p.account_id)}>キャンセル</button>}
      ...
      {tab === "tables" && (
        <div style={S.body}>
          ...
          {[...pendingMatches].sort((a, b) => {
            ...
            return (
              <div key={m.id} style={{ background: isMyMatch ? "#fff8f7" : C.card, border: `2px solid ${isMyMatch ? C.accent : C.border}`, borderRadius: 14, padding: "18px 16px", marginBottom: 12, boxShadow: isMyMatch ? `0 0 16px ${C.accent}22` : "0 1px 6px #00000010", position: "relative" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  <div style={{ flex: 1, textAlign: "center", background: C.dim, borderRadius: 10, padding: "12px 8px", border: `1px solid ${C.border}` }}>
                    <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 4, color: C.text }}>{m.p1_name}</div>
                    <div style={{ color: C.muted, fontSize: 12 }}>{m.p1_rating_before} pt</div>
                  </div>
                  <div style={{ flexShrink: 0, textAlign: "center", width: 32, color: C.muted, fontSize: 16, fontWeight: 900 }}>VS</div>
                  <div style={{ flex: 1, textAlign: "center", background: C.dim, borderRadius: 10, padding: "12px 8px", border: `1px solid ${C.border}` }}>
                    <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 4, color: C.text }}>{m.p2_name}</div>
                    <div style={{ color: C.muted, fontSize: 12 }}>{m.p2_rating_before} pt</div>
                  </div>
                </div>
                ...
                return (
                  <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {isCreator && <span style={{ fontSize: 10, fontWeight: 800, color: C.accent, background: "#fff0ee", border: `1px solid ${C.accent}44`, borderRadius: 4, padding: "1px 6px", marginLeft: 6 }}>作成者</span>}
                      {!isCreator && isOp && <span style={{ fontSize: 10, fontWeight: 800, color: C.orange, background: "#fff8ee", border: `1px solid ${C.orange}44`, borderRadius: 4, padding: "1px 6px", marginLeft: 6 }}>運営</span>}
                    </div>
                    {!isCreator && (
                      <button
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5, flexWrap: "wrap" }}>
                      <span style={{ fontWeight: 700, fontSize: 14, color: C.text }}>{p.account.name}</span>
                      {isMe && <span style={{ fontSize: 10, fontWeight: 800, color: C.accent, background: "#fff0ee", border: "1px solid #e8321a", borderRadius: 4, padding: "1px 6px" }}>YOU</span>}
                    ...
                    <div style={{ fontSize: 12, color: C.muted }}>{p.wins}勝 {p.losses}敗</div>
                  </div>
            ...
            <div key={m.id} style={S.card}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: C.accent, background: "#fff0ee", border: `1px solid ${C.accent}`, borderRadius: 12, padding: "2px 9px" }}>台{m.table_num}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: C.orange }}>{m.result}</span>
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                {[{ name: m.p1_name, before: m.p1_rating_before, after: m.p1_rating_after },
                  { name: m.p2_name, before: m.p2_rating_before, after: m.p2_rating_after }
                ].reduce((acc, p, i) => {
                  const diff = (p.after || 0) - (p.before || 0);
                  acc.push(
                    <div key={i} style={{ flex: 1, textAlign: "center", background: C.dim, borderRadius: 8, padding: "10px 6px" }}>
                      <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2, color: C.text }}>{p.name}</div>
                      <div style={{ fontSize: 11, color: C.muted }}>{p.before} →</div>
                      <div style={{ fontWeight: 800, fontSize: 15, color: diff >= 0 ? C.green : C.red }}>
                        {p.after ?? "—"}{p.after != null && <span style={{ fontSize: 11 }}> ({diff >= 0 ? "+" : ""}{diff})</span>}
                      </div>
                    </div>
                  );
                  if (i === 0) acc.push(<div key="vs" style={{ color: C.muted, fontSize: 14, fontWeight: 700, flexShrink: 0 }}>VS</div>);
                  return acc;
                }, [])}
              </div>
            </div>
      ...
      {/* ── マイページ ── */}
      {tab === "mypage" && (() => {
        const myMatches = matchHistory.filter(m => m.p1_id === account.id || m.p2_id === account.id);
        return (
          <div style={S.body}>
            {/* 今大会の成績 */}
            <div style={{ ...S.card, border: `2px solid ${C.accent}`, marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
                <img src="/logo.jpg" alt="POWERS" style={{ width: 48, height: 48, objectFit: "contain", borderRadius: 10, flexShrink: 0 }} />
                <div>
                  <div style={{ fontWeight: 800, fontSize: 18, color: C.text }}>{account.name}</div>
                  <div style={{ fontSize: 12, color: C.muted }}>{tournament.name}</div>
                </div>
              </div>
              {myPart ? (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {[["今大会レート", `${myPart.current_rating} pt`, C.accent], ["勝", myPart.wins, C.green], ["敗", myPart.losses, C.red]].map(([label, val, color]) => (
                    <div key={label} style={{ background: C.dim, borderRadius: 10, padding: "12px 8px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, color: C.muted }}>この大会には参加していません</div>
              )}
            </div>

            {/* 今大会の対戦履歴 */}
            <div style={S.sectionTitle}>今大会の対戦履歴（{myMatches.length}試合）</div>
            {myMatches.length === 0 && <div style={{ textAlign: "center", color: C.muted, padding: "24px 0" }}>まだ対戦記録がありません</div>}
            {myMatches.map(m => {
              const isP1 = m.p1_id === account.id;
              ...
              const diff = (myRatingAfter || 0) - (myRatingBefore || 0);
              const isWin = m.result?.includes(account.name) && m.result?.includes("勝利");
              const isAbort = m.result === "対戦中止";
              return (
                <div key={m.id} style={{ ...S.card, borderLeft: `4px solid ${isAbort ? C.muted : isWin ? C.green : C.red}`, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 14, color: isAbort ? C.muted : isWin ? C.green : C.red, marginBottom: 2 }}>
                        {isAbort ? "中止" : isWin ? "勝利" : "敗北"}
                      </div>
                      <div style={{ fontSize: 12, color: C.muted }}>vs {oppName} ／ 台{m.table_num}</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontWeight: 800, fontSize: 16, color: diff >= 0 ? C.green : C.red }}>{diff >= 0 ? `+${diff}` : diff}</div>
                      <div style={{ fontSize: 11, color: C.muted }}>{myRatingBefore} → {myRatingAfter ?? "—"}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            return (
              <>
                ...
                {mvpWins && mvpWins.wins > 0 && (
                  <div style={{ ...S.card, borderColor: C.orange, background: "#fffbf0", marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
                            ...
                            {isMe && <span style={{ fontSize: 10, fontWeight: 800, color: C.accent, background: "#fff0ee", border: `1px solid ${C.accent}`, borderRadius: 4, padding: "1px 6px" }}>YOU</span>}
                          </div>
