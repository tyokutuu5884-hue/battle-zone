...
// ─── Match Card Effects ───────────────────────────────────────
const EFFECT_CSS = `
@keyframes bz-float-up {
  0%   { transform: translateY(0) scale(1) rotate(var(--r)); opacity: 0.9; }
  100% { transform: translateY(-80px) scale(0.3) rotate(calc(var(--r) + 40deg)); opacity: 0; }
}
@keyframes bz-spark {
  0%   { transform: translate(0,0) scale(1); opacity: 1; }
  100% { transform: translate(var(--dx), var(--dy)) scale(0); opacity: 0; }
}
@keyframes bz-pulse-border {
  0%, 100% { box-shadow: 0 0 12px #ff442244, 0 0 0px #ff8c0000; }
  50%       { box-shadow: 0 0 28px #ff442288, 0 0 40px #ff8c0033; }
}
...
.bz-match-card {
  animation: bz-pulse-border 2.4s ease-in-out infinite;
}
.bz-particle { position: absolute; pointer-events: none; user-select: none; }
.bz-fire { animation: bz-float-up var(--dur) ease-out infinite; animation-delay: var(--delay); }
.bz-spark-el { animation: bz-spark var(--dur) ease-out infinite; animation-delay: var(--delay); }
`;

function InjectCSS() {
  useEffect(() => {
    const el = document.createElement("style");
    el.textContent = EFFECT_CSS;
...
// 炎パーティクル（カード下部から浮き上がる）
const FIRE_EMOJIS = ["🔥","🔥","🔥","✨","💥","🔥"];
const SPARK_EMOJIS = ["⚡","⚡","✦","⚡"];

function MatchCardEffects({ isMyMatch }) {
  const fires = useMemo(() => Array.from({ length: 8 }, (_, i) => ({
    id: i,
    left: `${8 + i * 11}%`,
    emoji: FIRE_EMOJIS[i % FIRE_EMOJIS.length],
    dur: `${1.8 + (i * 0.31) % 1.4}s`,
    delay: `${(i * 0.27) % 1.6}s`,
    rot: `${-20 + (i * 13) % 40}deg`,
    size: 14 + (i * 3) % 8,
  })), []);
  ...
  return (
    <>
      {/* 炎：カード下部 */}
      {fires.map(f => (
        <span key={f.id} className="bz-particle bz-fire" style={{
          bottom: -6, left: f.left,
          fontSize: f.size,
          "--dur": f.dur, "--delay": f.delay, "--r": f.rot,
        }}>{f.emoji}</span>
      ))}
      {/* 稲妻：カード内部 */}
      {sparks.map(s => (
        <span key={s.id} className="bz-particle bz-spark-el" style={{
      ...
      {tab === "tables" && (
        <div style={S.body}>
          ...
          {[...pendingMatches].sort((a, b) => {
            ...
            return (
              <div key={m.id} className={isMyMatch ? "bz-match-card" : ""} style={{ background: isMyMatch ? "#161a28" : "#12151f", border: `2px solid ${isMyMatch ? C.accent : C.border2}`, borderRadius: 14, padding: "18px 16px", marginBottom: 12, position: "relative", overflow: "hidden" }}>
                {isMyMatch && <MatchCardEffects isMyMatch={isMyMatch} />}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  <div style={{ color: C.muted, fontSize: 16, flexShrink: 0, textAlign: "center", width: 32 }}>VS</div>
                  <div style={{ flex: 1, textAlign: "center", background: "#161a28", borderRadius: 10, padding: "12px 8px", border: `1px solid ${C.border}` }}>
                ...
                {(isMyMatch || isOperator) ? (
                  <div>
                    {(() => {
                      ...
                      const otherReport = isP1 ? m.p2_report : isP2 ? m.p1_report : null;
                      // 自分視点の勝ち = p1なら"p1"、p2なら"p2"
                      ...
                      return (
                        <div>
                          {/* 報告状況 */}
                          {!isOperator && (m.p1_report || m.p2_report) && (
                          ...
                          {/* 不一致警告 */}
                          {m.p1_report && m.p2_report && m.p1_report !== m.p2_report && (
